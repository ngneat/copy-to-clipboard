...
function injectImports(options: Schema): Rule {
  return (host: Tree, context: SchematicContext) => {
    ...
    const change = insertImport(moduleSource, modulePath, 'CopyToClipboardModule', '@ngneat/copy-to-clipboard');
...
function addModuleToImports(options: Schema): Rule {
  return (host: Tree, context: SchematicContext) => {
    ...
    const moduleImport = `CopyToClipboardModule`;

    addModuleImportToRootModule(host, moduleImport, null as any, project);

    context.logger.log('info', `🌈 @ngneat/copy-to-clipboard is imported`);
...
function log(): Rule {
  return (host: Tree, context: SchematicContext) => {
    context.logger.log('info', `👏 @ngneat/copy-to-clipboard ready to use`);
